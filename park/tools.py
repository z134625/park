    ...
    if hasattr(obj, 'paras') and isinstance(obj.paras, Paras):
        ...
        for key, value in obj.paras._set_list:
            ...
            if not has:
                ...
                if (warn and obj.paras._warn) and (cover and obj.paras._cover) and not self:
                    warnings.warn("当前设置重复的属性(%s)，将覆盖该属性" % key, RuntimeWarning, stacklevel=2)
...
class Paras:
    ...
    def __init__(self, allow: bool = True, **kwargs):
        ...
        self._set_paras(allow=allow, **set_dict)
    ...
    def _set_paras(self, allow: bool = True, **kwargs):
        self._allow = allow
        try:
            if '_attrs' in kwargs and '_set_list' not in kwargs:
                kwargs.update({
                    '_set_list': list(kwargs.get('_attrs', {}).items())
                })
    ...
    def update(self, kwargs: dict) -> Any:
        self._set_paras(allow=True, **kwargs)
        return self
...
class SettingParas(Paras):
    ...
    """
    ban = ['_attrs', '_suffix_ini', 'root_func', 'grant']
    ...
    def with_paras(self, gl: bool = False, **kwargs) -> Any:
        ...
        if not gl:
            ...
            if dif_set.__len__() >= 1:
                if kwargs.get('_warn', False) or self.paras._warn:
            ...
            obj.paras = paras
            obj.paras._set_list = list(obj.paras._attrs.items())

            return setAttrs(obj, self=True, **paras_dict)
        else:
            self.paras.update(kwargs)
            return self
    ...
    def open(self, path: str, **kwargs) -> Any:
        ...
        if suffix == '.py':
            with open(path, 'r', encoding=kwargs.get("encoding", None)) as f:
                ...
                self.paras._set_list += list(d.items())
        elif suffix == '.json':
            with open(path, 'r', encoding=kwargs.get("encoding", None)) as f:
                # self.paras._set_dict = {**self.paras._set_dict, **json.load(f)}
                self.paras._set_list += list(json.load(f).items())
                f.close()
